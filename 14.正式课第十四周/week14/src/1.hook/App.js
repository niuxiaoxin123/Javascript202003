...
//     let divRef= useRef();
//     console.log(divRef);
//     // 把ref加到哪个元素的属性上，那么就把这个元素赋值给对应current属性;
//     // 如果给两个不同的元素ref都加spanRef；那么后面的元素会把前面的覆盖掉；
//     return <div>
...
//     });
//     // 在class定义类中有状态， 生命周期
//     // 在初始化组件时，会默认调用useEffect里面的回调函数；相当于componentDidMount；当数据更新时，那么也会执行该回调函数；相当于componentDidUpdate;
//     useEffect(()=>{
//         // 设置依赖项；没有设置依赖项，那么只要state中数据发生变化，都会执行
//         console.log("雪斌很有钱");
//     },[state.m]);
//     useEffect(()=>{
//         // 设置依赖项；没有设置依赖项，那么只要state中数据发生变化，都会执行
